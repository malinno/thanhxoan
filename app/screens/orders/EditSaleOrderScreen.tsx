...
const EditSaleOrderScreen: FC<Props> = ({ route, ...props }) => {
  const navigation = useNavigation();
  const user = useAuth(state => state.user);

  const mutation = editSaleOrderMutation();
  ...
  const { data: partner } = useCustomerDetail(order?.partner_id?.[0]);
  ...
  useEffect(() => {
    if (mutation.isPending || isRefetching) Spinner.show();
    else Spinner.hide();

    return () => {
      Spinner.hide();
    };
  }, [mutation.isPending || isRefetching]);
  ...
  const onOrderEdited = (result: SaleOrder) => {
  ...
  return (
    <View style={styles.container}>
      <Header
        title={order?.name || 'Thông tin khách hàng'}
...
export default EditSaleOrderScreen;
