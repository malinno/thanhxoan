...
interface Props extends ViewProps {
  index: number;
  data: PosOrderLine;
  onChange?: (index: number, data: Partial<PosOrderLine>) => void;
  onRemove?: (index: number, data: PosOrderLine) => void;
}

const PosOrderProductItem: FC<Props> = ({
  index,
  data,
  onChange,
  onRemove,
  ...
  const isPromotionLine =
    data.is_dong_ctkm_chiet_khau ||
    data.is_dong_ctkm_tang_kem ||
    data.is_sanpham_khuyen_mai ||
    data.is_dongchietkhautongdon;

  const onChangeQty = (qty: string) => {
    if (Number(qty) > Number(data?.product_free_qty)) {
      toast.show(`Số lượng tối đa là ${data?.product_free_qty}`, {
        placement: 'bottom',
      });
      return onChange?.(index, {
        product_uom_qty: Number(data?.product_free_qty),
      });
    }
    onChange?.(index, {
      // ...data,
