...
const RoutePlanStateItem: FC<Props> = memo(props => {
  const { index, data, onRemove, removable } = props;

  const _onRemove = () => onRemove?.(index, data);

  return (
    <Animated.View style={[styles.item]} exiting={FadeOut.springify()}>
...
RoutePlanStateItem.defaultProps = {
  removable: true,
};

export default RoutePlanStateItem;
