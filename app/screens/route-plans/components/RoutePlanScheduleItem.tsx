...
export type RouteScheduleItemViewModel = ErpRouteSchedule & {
  distance?: string;
};
...
interface Props extends Omit<TouchableProps, 'onPress'> {
  ...
  onPress?: (data: RouteScheduleItemViewModel) => void;
}

const RoutePlanScheduleItem: FC<Props> = React.memo(
  ({ index, data, onPress, style, ...props }) => {
    ...
    const onPressPosOrders = () => {
      if (!data.store_id?.id) return;
      navigation.navigate(SCREEN.CREATE_POS_ORDER, {
        partnerId: data.store_id?.id,
      });
    };
    ...
    return (
      <Touchable
        ...
        <HStack style={styles.header}>
          ...
          <CustomerCategoryView category={data?.store_id?.category} />
        </HStack>
        <View
          style={[
            styles.separator,
        ...
        <View style={styles.body}>
          ...
          <HStack style={styles.row}>
            ...
            <Text style={[styles.text]} numberOfLines={1}>
              {data.router_plan_id?.[1]} - {data.router_id?.[1]}
            </Text>
          </HStack>
          <HStack style={styles.row}>
...
export default RoutePlanScheduleItem;
